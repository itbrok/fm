...
?>
    ...
    f.submit();" href="#"><i class="fa fa-download"></i> <?php echo lng('Download') ?></a>
        ...
        if ($is_text && $isNormalEditor) {
            ...
            echo '<div id="editor" style="height: 84vh; border: 1px solid grey;"></div>';
...
<?php
    echo '</div>'; // close col-md-9 for editor content
    echo '</div>'; // close row for explorer+editor
    fm_show_footer();
    exit;
}
...
function fm_generate_explorer_html_recursive($scan_abs_path, $base_root_path) {
    ...
    foreach ($items as $item) {
        ...
        if (!FM_SHOW_HIDDEN && substr($item, 0, 1) === '.') {
            continue;
        }
...
function fm_generate_explorer_html_recursive_toplevel($scan_abs_path, $base_root_path) {
    ...
    foreach ($items as $item) {
        ...
        if (is_dir($item_abs_path_for_scan)) {
            ...
            $temp_files[$item] = $item_fm_path_for_link;
        }
    }
    ...
    <head>
        ...
        <style>
            ...
            *::after {
            ...
            #file-explorer-container {
                height: 84vh; /* Adjust as needed */
                overflow-y: auto;
                border-right: 1px solid #ddd;
                padding-top: 10px; /* Align with main content's pt-3 */
            }
            ...
            #file-explorer-container li {
                padding: 2px 0;
            }

            #file-explorer-container i.fa {
                margin-right: 5px;
            }
            .folder-toggle {
                cursor: pointer;
            ...
            ul.sub-explorer {
                ...
                padding-left: 15px; /* Indent sub-folders */
            }
        ...
        if (FM_THEME == "dark"): ?>
            <style>
                ...
                .theme-dark #file-explorer-container {
                    border-right-color: #454d55; /* Darker border for dark theme */
                }
            </style>
        <?php endif; ?>
    </head>
                    ...
                    if ($.fn.dataTable) {
                        ...
                        if ($table.length && !$.fn.dataTable.isDataTable($table)) { // Check if table exists and not already initialized
                            ...
                            mainTable = $table.DataTable({
                             ...
                             // filter table - ensure this is also re-applied if table is initialized late
                            ...
                            console.error("DataTables plugin still not loaded after delay.");
                        }
                    }
                }
        ...
        ?>
            <script>
                ...
                require(['vs/editor/editor.main'], function() {
                    ...
                    // monacoEditor.addCommand(monaco.KeyMod.CtrlCmd | monaco.KeyCode.KEY_S, null); // May need a more specific way to clear if set

                    // Save command using DOM keydown event listener
                    var editorDomNode = monacoEditor.getDomNode();
                    if (editorDomNode) {
                        editorDomNode.addEventListener('keydown', function(event) {
                            if ((event.ctrlKey || event.metaKey) && event.key === 's') {
                                if (typeof event.preventDefault === 'function') {
                                    event.preventDefault();
                                }
                                edit_save(null, 'monaco');
                            }
                        });
                    }
